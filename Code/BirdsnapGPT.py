...
# birdsnapplus = json.load(open("birdsnap_with_loc_2019.json"))
...
df = []
cols = ["url", "common_name", "scientific_name", "GPT_response"]
...
for i in range(0, len(testbirdsnap.index)):
    ...
    #         df2.to_csv("Birdsnap_GPTFinished" + str(i) + ".csv", sep='\t', index=False)
    #     continue
    #Long prompt
    try: 
        response = llm.chat.completions.create(
...
print(df)
df.to_csv("Birdsnap_GPTFinished.csv", sep='\t', index=False)
